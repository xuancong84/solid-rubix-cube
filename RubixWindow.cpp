...
char filename[1024];
OPENFILENAME g_ofn={
	sizeof(OPENFILENAME),NULL,base::hInst,
...
BOOL CALLBACK RubixWindow::m_DialogProc( HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_INITDIALOG:
		Init:
		SetWindowText(hDlg, StringTable(114).c_str());
		for(int x=0, X=sizeof(fields)/sizeof(int)/2; x<X; x++)
			SetDlgItemText(hDlg, fields[x][0], StringTable(fields[x][1]).c_str());
		...
		SetDlgItemText(hDlg, IDS_denom, ("/"+ToString(pRubix->history.size())).c_str());
		break;
	case WM_COMMAND:
...
void RubixWindow::Update(double time_ms, int stack)
{
	if(rotation_angle!=-1 && !(selected_cube_index>=0 && (mouse_button_state&1)))
	{
		...
		if(rotation_angle==0 || rotation_angle>=90)
		{
			...
			SetDlgItemInt(base::hDlg, IDC_gotohist, pRubix->history_posi, false);
			SetDlgItemText(base::hDlg, IDS_denom, ("/"+ToString(pRubix->history.size())).c_str());
		}
		dirty = max(dirty, 2);
	}
	if(blink)
...
int RubixWindow::DetectCollision(int mouse_hit_x, int mouse_hit_y)
{
	...
	if(cube_id>=0){
		...
		if(GetAsyncKeyState(VK_SHIFT)<0){
			...
			if(cube->is_boxed()){
				cube->clear_box();
				pRubix->boxed_indices.erase(cube);
			}else{
				cube->create_box();
				pRubix->boxed_indices.insert(cube);
			}
		}
	}
